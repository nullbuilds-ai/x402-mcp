...
export async function estimatePayment(input: EstimateInput): Promise<string> {
  let res: Response;
  try {
    ...
    return `Failed to reach ${input.resource_url}: ${(err as Error).message}`;
  }
  ...
  try {
    payload = decodePaymentRequired(header) as PaymentRequiredPayload;
  } catch {
  ...
  accepts.forEach((a, i) => {
    ...
    if (a.payTo) lines.push(`- Recipient: ${a.payTo}`);
    if (a.description) lines.push(`- Description: ${a.description}`);
    if (a.maxTimeoutSeconds) lines.push(`- Timeout: ${a.maxTimeoutSeconds}s`);
  });
